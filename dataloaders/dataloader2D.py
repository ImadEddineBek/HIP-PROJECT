...
class HipLandmarksDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, transform=None):
        ...
        self.mri_map = {'PD': 0, 'T1': 1, 'T2': 2}
        self.mri_rev_map = {v: k for k, v in self.mri_map.items()}
    ...
    def __getitem__(self, idx):
        ...
        MRI_sequence = self.landmarks_frame['MRI_sequence'][idx]
        sample = {'image': image, 'landmarks': landmarks}
        if self.transform:
            sample = self.transform(sample)

        return sample['image'], sample['landmarks'], self.mri_map[MRI_sequence]
...
class HipJigsaw(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample['image'], sample['indexes'], self.mri_map[MRI_sequence]
...
def get_dataloader2D(config):
    ...
    transform_test = transforms.Compose([
        # transforms.Resize(config.image_size),
        ToTensorTest(config.image_size),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
    ...
    train_loader = DataLoader(dataset=dataset_train, batch_size=1, shuffle=False,
...
def get_dataloader2D_(config):
    transform_train = transforms.Compose([
        # transforms.ToPILImage(),
        # transforms.Resize(config.image_size),
        ToTensor_(config.image_size),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
    transform_test = transforms.Compose([
        # transforms.Resize(config.image_size),
        ToTensorTest_(config.image_size),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
...
def get_dataloader2DJigSaw(config):
    transform_train = transforms.Compose([
        # transforms.ToPILImage(),
        # transforms.Resize(config.image_size),
        ToTensorJigsaw(config.image_size),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
    transform_test = transforms.Compose([
        # transforms.Resize(config.image_size),
        ToTensorJigsawTest(config.image_size),
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
...
if __name__ == '__main__':
    ...
    for i in range(len(dataset)):
        ...
        print(i, sample[0].shape, sample[1].shape, dataset.mri_rev_map[sample[2]])
